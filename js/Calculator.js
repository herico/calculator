...
class Calculator {
   ...
   * @param {*} expression
   */
  evaluateMathExpression(isEqualOperator) {
    this.expression = eval(this.expression) + "";
    if (isEqualOperator) {
      let result = Number(this.expression);
      if (parseInt(result) !== result) {
        this.expression = parseFloat(result.toFixed(1)) + "";
      }
    }
  }
  ...
  deleteCharacter() {
    if (this._isExpressionEmpty()) {
      ...
      if (this._isThereADot()) {
        this.isDotCharHasBeenUsedInThisNumber = !this
          .isDotCharHasBeenUsedInThisNumber
          ? true
          : this.isDotCharHasBeenUsedInThisNumber;
      }
    }
  }
  ...
  _setNumber(value) {
    if (this.isEqualOperatorHasBeenUsed) {
      ...
      this._isExpressionEmpty() ||
      !this._isLastCharNotANumber() ||
      this._getLastChar() === "."
    ) {
      this.expression += value;
  ...
  _setOperator(value) {
    if (this._isExpressionEmpty()) {
      ...
      this._getLastChar() !== "." &&
      !this._isExpressionEmpty() &&
      this._isThereAnOperatorInTheExpression() &&
      !this.isEqualOperatorHasBeenUsed
    ) {
      this.evaluateMathExpression(true);
      this.isDotCharHasBeenUsedInThisNumber = false;
      ...
      this.expression =
        this.expression.substr(0, this.expression.length - 2) + value + " ";
      this.isEqualOperatorHasBeenUsed = false;
    }
  }
